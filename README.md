...
Написал тестирующую программу на питоне. Она запускает программу на ассемблере, прогоняет её на заготовленных тестах и выводит результат в консоль. Для её корректной работы файлы IHW1.asm и IHW1_lib.asm должны находиться в одном каталоге с test_program.py.
